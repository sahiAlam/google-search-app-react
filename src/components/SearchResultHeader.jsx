...
const SearchResultHeader = () => {
  ...
  //  switch tab
  const switchTabHandler = (menuItem) => {
  ...
  return (
    <>
      <div className="p-[15px] pb-0 md:pr-5 md:pl-5 md:pt-7 border-b border-[#ebebeb] flex md:block flex-col items-center sticky top-0 bg-white">
        ...
        <div className="flex ml-[-12px] mt-3">
          {menu.map((menu, index) => (
            <span
              key={index}
              className={`flex items-center p-3 relative text-[#5f6368] cursor-pointer ${
                selectedMenu === menu.name ? "text-[#1a73e8]" : ""
              }`}
              onClick={() => switchTabHandler(menu)}
            >
